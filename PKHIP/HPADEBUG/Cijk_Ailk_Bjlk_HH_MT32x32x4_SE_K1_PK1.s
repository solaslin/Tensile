...
.text
.protected Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1
.globl Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1
.p2align 8
.type Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1,@function
.section .rodata,#alloc
.p2align 6
.amdhsa_kernel Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1
...
amdhsa.version:
  ...
  - 0
amdhsa.kernels:
  - .name: Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1
    .symbol: 'Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1.kd'
    .language:                   OpenCL C
    .language_version:
      - 2
      - 0
...
...
.end_amdgpu_metadata
Cijk_Ailk_Bjlk_HH_MT32x32x4_SE_K1_PK1:
...
v_cmp_lt_u32 s[68:69], v18, v16                    // wgMT < MT
v_cndmask_b32 v18, v16, v18, s[68:69]              // wgMT = (wgMT < MT) ? wgMT : MT
...
v_cmp_eq_u32 s[68:69], v28, v23                    // mask
v_mov_b32 v16, s68                                 // 
v_mov_b32 v17, s69                                 // 
...
/* shift d1 r=1 v=0 */
label_0014:
_v_cmpx_eq_u32 s[68:69], v28, v23                  // serial % SG == (wgMT/VECTOR_WIDTH)%SG
// src=4, dst=0
...
_v_add_co_u32 v16, vcc, s68, v16                   // coord0 = tid0*VW + wg0*MT0
s_mul_i32 s70, 0x20, s[sgprWorkGroup1]             // <- wg1*MT1
_v_add_co_u32 v17, vcc, s70, v17                   // coord1 = tid1*VW + wg1*MT1
...
// Ethan: HPA Can Skip Below First //
...
/* calc coords, apply mask, and issue loads (if necessary) */
/* (d1,vc1,d0,vc0)=(0,0,0,0) */
v_cmp_lt_u32 s[68:69], v16, s[sgprSizeI]           // coord0 < size0
...
v_cndmask_b32 v21, -1, v21, s[74:75]               // clip if OOB. offset
/* (d1,vc1,d0,vc0)=(0,0,1,0) */
_v_add_co_u32 v19, vcc, v16, 16                    // coord0.1: coord0 += d0*sg0*VW + vc0
v_cmp_lt_u32 s[68:69], v19, s[sgprSizeI]           // coord0 < size0
...
v_cndmask_b32 v22, -1, v22, s[76:77]               // clip if OOB. offset
/* (d1,vc1,d0,vc0)=(0,1,0,0) */
...
v_cndmask_b32 v23, -1, v23, s[78:79]               // clip if OOB. offset
/* (d1,vc1,d0,vc0)=(0,1,1,0) */
_v_add_co_u32 v19, vcc, v16, 16                    // coord0.1: coord0 += d0*sg0*VW + vc0
v_cmp_lt_u32 s[68:69], v19, s[sgprSizeI]           // coord0 < size0
...
v_cndmask_b32 v28, -1, v28, s[82:83]               // clip if OOB. offset
/* (d1,vc1,d0,vc0)=(1,0,1,0) */
_v_add_co_u32 v19, vcc, v16, 16                    // coord0.1: coord0 += d0*sg0*VW + vc0
v_cmp_lt_u32 s[68:69], v19, s[sgprSizeI]           // coord0 < size0
...
v_cndmask_b32 v29, -1, v29, s[84:85]               // clip if OOB. offset
